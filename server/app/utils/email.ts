...
module.exports = (app) => {
  const { EMAIL_NOTICE } = app.config;

  // create reusable transporter object using the default SMTP transport
  // EMAIL_NOTICE.transporterConfig.auth.pass = decrypt(EMAIL_NOTICE.transporterConfig.auth.pass)
   ...
   */
  const getEmailList = (target, userId) => {
  ...
  return {
     ...
     * @example sendMail({ receiver: '494657028@qq.com', html: 'hellllll' })
     */
    sendEmail: async ({ receiver, html, subject, text }) => {
    ...
    getEmailData: (article, disscussData, userId) => {
